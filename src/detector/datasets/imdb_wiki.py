...
def get_loader(IMDB_dataset_path):
    ...
    print(os.path.join(IMDB_dataset_path, 'wiki.mat'))
    meta = loadmat(os.path.join(IMDB_dataset_path, 'wiki.mat'), squeeze_me=True)
    
    _gener_mapper = { 0:'female', 1:'male' }
    
    zipped = zip(meta['wiki']['full_path'].item(),
                 meta['wiki']['face_score'].item(),
                 meta['wiki']['face_location'].item(),
                 meta['wiki']['gender'].item())
