...
def model(inputs_image, output_grids, is_training_mode, dropout_keep_prob, reuse=False):
    ...
    with tf.variable_scope('face-net', reuse=reuse):    
        ...
        net = bottleneck(net, 24, t=6, stride=1, scope='bottleneck-2-1', reuse=reuse)
        outputs_dict[(int(net.shape[1]), int(net.shape[2]))] = net
        net = bottleneck(net, 24, t=6, stride=2, scope='bottleneck-2-2', reuse=reuse)
        outputs_dict[(int(net.shape[1]), int(net.shape[2]))] = net

        net = bottleneck(net, 32, t=6, stride=1, scope='bottleneck-3-1', reuse=reuse)
        outputs_dict[(int(net.shape[1]), int(net.shape[2]))] = net
        net = bottleneck(net, 32, t=6, stride=1, scope='bottleneck-3-2', reuse=reuse)
        outputs_dict[(int(net.shape[1]), int(net.shape[2]))] = net
        net = bottleneck(net, 32, t=6, stride=2, scope='bottleneck-3-3', reuse=reuse)
        outputs_dict[(int(net.shape[1]), int(net.shape[2]))] = net

        net = bottleneck(net, 64, t=6, stride=1, scope='bottleneck-4-1', reuse=reuse)
        outputs_dict[(int(net.shape[1]), int(net.shape[2]))] = net
        net = bottleneck(net, 64, t=6, stride=1, scope='bottleneck-4-2', reuse=reuse)
        outputs_dict[(int(net.shape[1]), int(net.shape[2]))] = net
        net = bottleneck(net, 64, t=6, stride=1, scope='bottleneck-4-3', reuse=reuse)
        outputs_dict[(int(net.shape[1]), int(net.shape[2]))] = net
        net = bottleneck(net, 64, t=6, stride=2, scope='bottleneck-4-4', reuse=reuse)
        outputs_dict[(int(net.shape[1]), int(net.shape[2]))] = net

        net = bottleneck(net, 96, t=6, stride=1, scope='bottleneck-5-1', reuse=reuse)
        outputs_dict[(int(net.shape[1]), int(net.shape[2]))] = net
        net = bottleneck(net, 96, t=6, stride=1, scope='bottleneck-5-2', reuse=reuse)
        outputs_dict[(int(net.shape[1]), int(net.shape[2]))] = net
        net = bottleneck(net, 96, t=6, stride=2, scope='bottleneck-5-3', reuse=reuse)
        outputs_dict[(int(net.shape[1]), int(net.shape[2]))] = net
